...
public class H2DbSupport extends DbSupport {
		...
		return getSQLHandler()
				.getItemAsLong(
						"select CURRENT_VALUE from "
								+ "INFORMATION_SCHEMA.SEQUENCES where SEQUENCE_SCHEMA = '"
								+ getSchemaName() + "' and SEQUENCE_NAME = '"
								+ sequenceName + "'");
	}
	...
	public void incrementIdentityColumnToValue(String tableName,
		...
		getSQLHandler().executeUpdate(
				"alter table " + qualified(tableName) + " alter column "
						+ quoted(identityColumnName) + " RESTART WITH "
						+ identityValue);
	}
	...
	protected void disableCheckAndUniqueConstraints() {
		...
		try {
			...
			throw new UnitilsException(
					"Error while disabling check and unique "
	...
	protected void disableNotNullConstraints() {
		...
		try {
			...
			resultSet = queryStatement
					.executeQuery("select col.TABLE_NAME, "
			...
			while (resultSet.next()) {
				...
				alterStatement.executeUpdate("alter table "
						+ qualified(tableName) + " alter column "
